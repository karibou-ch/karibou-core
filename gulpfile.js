...
gulp.task('build', function () {
  gulp.src(buildConfig.mockFiles)
    .pipe(concat('karibou-core-mocks.js'))
  ...
  return gulp.src(buildConfig.pluginFiles)
    .pipe(concat('karibou-core.js'))
    .pipe(header(buildConfig.closureStart))
    .pipe(footer(buildConfig.closureEnd))
    .pipe(header(buildConfig.banner))
    .pipe(gulp.dest(buildConfig.dist))
    .pipe(gulp.dest(buildConfig.demo.karibouCore))
    .pipe(uglify())
    .pipe(header(buildConfig.banner))
...
function makeChangelog(version) {
  ...
  changelog({
    repository: 'https://github.com/karibou-ch/karibou-core',
    version: version,
    file: file
  }, function (err, log) {
